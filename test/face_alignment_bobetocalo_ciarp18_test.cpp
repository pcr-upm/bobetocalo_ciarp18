/** ****************************************************************************
 *  @file    face_alignment_bobetocalo_ciarp18_test.cpp
  ...
  )
{
  ...
  for (int cont=0; cont < upm::DB_LANDMARKS.size(); cont++)
  {
    ...
    if (feature_idx < 1)
      continue;
    for (const std::pair<upm::FacePartLabel,std::vector<int>> &part : upm::DB_PARTS)
  ...
  boost::shared_ptr<upm::FaceAlignment> fa(new upm::FaceAlignmentCrn("data/"));
...
//  viewer->init(0, 0, "face_alignment_bobetocalo_ciarp18_test");
//  showResults(viewer, ticks, 0, frame, composite, faces, ann);

  UPM_PRINT("End of face_alignment_bobetocalo_ciarp18_test");
  return EXIT_SUCCESS;
};
